...
@printf("%-15s | %-10s | %-10s | %-10s | %-12s\n", "Method", "Init (ms)", "Conv (ms)", "Total (ms)", "Alloc (MB)")
println("-"^70)

# --- Helper function for formatted output ---
function print_result(method::AbstractString, t_init::Float64, t_conv::Float64, alloc_bytes::Int)

    total = (method == "FFT" ? t_conv : t_init + t_conv)
    alloc_mb = alloc_bytes / 1024^2
    @printf("%-15s | %-10.3f | %-10.3f | %-10.3f | %-12.3f\n",
            method, t_init * 1000, t_conv * 1000, total * 1000, alloc_mb)
end
...
# --- 6. FFT Convolution ---
t_conv_fft = fft_conv_gpu_bench(image, kernel_size)
print_result("FFT-GPU", 0.0, t_conv_fft, 0)
